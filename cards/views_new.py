...
def start_analysis(request, card_uuid):
    """Start analysis for a specific card using the job queue (AJAX endpoint)."""
    try:
        card = analysis_manager.get_card_by_uuid(card_uuid)
        if not card:
        ...
        # Check if job already exists for this card
        ...
        if existing_job:
            return JsonResponse({
                ...
                'message': f'Analysis job already queued (Job: {existing_job["job_id"][:8]}...)',
                'job_id': existing_job['job_id']
            })
          # Get analysis type from request
        analysis_type = request.POST.get('analysis_type', 'full_analysis')
        component_type = request.POST.get('component_type', None)
        priority = int(request.POST.get('priority', 0))
        
        # Enqueue the job
        if analysis_type == 'single_component' and component_type:
            if component_type not in ALL_COMPONENT_TYPES:
                return JsonResponse({'error': 'Invalid component type'}, status=400)
            
            job_id = job_queue.enqueue_card_analysis(
                card_uuid, 
                priority=priority,
                job_type=f'single_component_{component_type}'
            )
        else:
            job_id = job_queue.enqueue_card_analysis(
                card_uuid, 
...
def analysis_dashboard(request):
    """Enhanced dashboard for monitoring analysis progress and job queue."""
    try:
        ...
        # Get recent jobs
        recent_jobs = job_queue.get_recent_jobs(limit=15)
