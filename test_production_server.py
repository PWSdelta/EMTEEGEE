...
def test_server_connectivity():
    ...
    try:
        ...
        sock.close()
        if result == 0:
            print(f"✅ Port 8001 is open and accessible")
        else:
            print(f"❌ Port 8001 is not accessible")
            return False
...
def test_worker_workflow():
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(f"✅ Worker registered successfully: {response.status_code}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Heartbeat successful: {response.status_code}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Work retrieved: {response.status_code}")
            work_data = response.json()
            print(f"   Work data: {json.dumps(work_data, indent=2)[:300]}...")
        elif response.status_code == 204:
            print(f"✅ No work available (expected): {response.status_code}")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Worker status retrieved: {response.status_code}")
...
def test_queue_management():
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Queue status retrieved: {response.status_code}")
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"✅ Workers list retrieved: {response.status_code}")
...
def generate_test_report(api_results):
    ...
    if successful_tests == total_tests:
        print("\n🎉 All tests passed! The production server is ready for swarm operations.")
    elif successful_tests > total_tests * 0.7:
        print("\n⚠️  Most tests passed. Some endpoints may need attention.")
    else:
        print("\n❌ Many tests failed. Server configuration may need review.")
